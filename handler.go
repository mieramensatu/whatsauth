...
func PostBalasan(w http.ResponseWriter, r *http.Request) {
	...
	if r.Header.Get("Secret") == os.Getenv("SECRET") {
		if msg.Message == "loc" || msg.Message == "Loc" || msg.Message == "lokasi" || msg.LiveLoc {
		...
		} else if msg.Message == "harganya" || msg.Message == "total"{
				dt := &wa.TextMessage{
					To:       msg.Phone_number,
					IsGroup:  false,
					Messages: fmt.Sprintf("makasiihh kakak %s harganya sesuai dengan yang ada di gambar ya kak", msg.Alias_name),
				}
				resp, _ = atapi.PostStructWithToken[atmessage.Response]("Token", os.Getenv("TOKEN"), dt, "https://api.wa.my.id/api/send/message/text")
			...
			randm := []string{
				"Hai Hai Haiii kamuuuui " + msg.Alias_name + "\ngadingnya lagi gaadaa \n aku scarabott salam kenall yaaaa \n Cara penggunaan WhatsAuth ada di link berikut ini ya kak...\n" + link,
				"IHHH jangan SPAAM berisik tau scara lagi tidur",
				"Kamu ganteng tau",
				...
				"Cihuyyyy hari ini scaraa bahagiaaa banget",
